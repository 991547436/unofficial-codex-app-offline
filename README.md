...
**Web-Only (all platforms):** Install Node.js 18+ and `npm install -g @openai/codex`, download `*-web.zip`, run `install.bat` / `bash install.sh` once, then run `start.bat` / `bash start.sh`. Stop with `stop.bat` / `bash stop.sh`; check status with `status.bat` / `bash status.sh`.
